% rap_cfg - example measurement configuration file -
%
...
def.varstep = -[2 1 0.5];       % [starting stepsize ... minimum stepsize] of the tracking variable
...
def.expvardescription = 'abs-coeff decrement';  % description of the tracking variable

% limits for experimental variable
def.minvar = -12;                 % minimum value of the tracking variable
...
def.exppar1 = [1 1];               % vector containing experimental parameters for which the exp is performed
def.exppar1unit = '';           % unit of experimental parameter
def.exppar1description = '';    % description of the experimental parameter
...
def.result_path = ['results', filesep];     % where to save results
def.control_path = ['control', filesep];    % where to save control files
...
def.intervallen = 44100;			% length of each signal-presentation interval in samples (might be overloaded in 'expname_set')
